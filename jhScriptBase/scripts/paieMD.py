...
class DataWrapper:
	...
	def getData(self, selList, startFrame, endFrame, dataType, attrsType, comments):
		...
		dateOfExport = cmds.date( format = "hh:mm-DD/MM/YY")
		if dataType == 'pose':
			clipLength = 1
		else:
	...
	def getObjDict(self, objFullPath, startFrame, endFrame, dataType, attrsType):
		...
		if cmds.objExists( objFullPath + '.rotateOrder'):
			objDict['objData']['rotateOrder'] = cmds.getAttr( objFullPath +'.rotateOrder' )
		else: objDict['objData']['rotateOrder'] = 0
		
		
		# add object attributes
		if attrsType == 'all':
			attrList = cmds.listAttr(objFullPath, unlocked=True, scalar=True, multi=True)
		else: #defaults to 'keyable' if none set
			attrList = cmds.listAttr(objFullPath, unlocked=True, keyable=True, visible=True, scalar=True, multi=True)
	...
	def getAttrDict(self, objFullPath, attrName, startFrame, endFrame, dataType):
		...
		attrDict = {'values': {} } 
		keyframeCount = cmds.keyframe(objName + '.' + attrName, time=( startFrame, endFrame ), q=True, keyframeCount=True)
		
		if keyframeCount != 0 and dataType == 'anim':
			# setting attribute anim data
			attrDict['values']['anim'] = {'animData': {}, 'animKeys': {} }
			attrDict['values']['anim']['animData']['weightedTangents'] = cmds.keyTangent(objName + '.' + attrName, time=( startFrame , endFrame ), q=True, wt=True)[0]
			...
			try:
				preInfinity, postInfinity = cmds.setInfinity(objName, attribute = attrName, query=True, pri=True, poi=True)
			except:
			...
			attrValue = cmds.getAttr(objFullPath + '.' + attrName)
	...
	def getKeyframeDict(self, objFullPath, attrName, keyframeCount, startFrame, endFrame):
		...
		attributes = cmds.keyTangent(objFullPath + '.' + attrName, time=( startFrame , endFrame ), q=True, inAngle=True, outAngle=True, inWeight=True, outWeight=True, inTangentType=True, outTangentType=True, lock=True, weightLock=True)
		frameValues = cmds.keyframe(objFullPath + '.' + attrName, time=( startFrame , endFrame ), query=True, absolute=True, timeChange=True, valueChange=True)
		
		#breakdown = cmds.keyframe(objFullPath + '.' + attrName, time=( startFrame , endFrame ), query=True, breakdown=True)
	...
	def writeToScene(self, selection, selectOrder, namespace, animOffset):
		...
		for i in mutualObjs:
			...
			try:
				if (cmds.keyframe(mutualObjs[i], time=( startTime , stopTime ), q=True, keyframeCount=True)):
					isKey = 1
		...
		if self.dataObj.getHeaderAttr('filetype') == 'anim':		
			if isKey:
				proceed = cmds.confirmDialog( title="Keys exist", message = ("Keys already exist in framerange: " + str(startTime) + '-' + str(stopTime) + '\nOverrwrite?'), button = ["Yes", "No"], defaultButton = "Yes", cancelButton = "No", dismissString = "No")
				...
				cmds.cutKey( objList , time=(startTime , stopTime), clear=True)
		...
		for objID in existObjIdIter:
			...
			if cmds.objExists( obj + '.rotateOrder'):
				currRoo = cmds.getAttr(obj + '.rotateOrder')
		...
		if len(rooMismatchDict.values()):
			...
			proceed = cmds.confirmDialog( title="Keys exist", 
			...
			if proceed == "Yes":
				...
				for key in rooMismatchDict:
					...
					try:
						cmds.setAttr(key + '.rotateOrder', rooMismatchDict[key]["sourceRoo"])
					except:
					...
					if self.dataObj.hasAnim(objID, attr):
						...
						for key in self.dataObj.getAttrKeyID(objID , attr):
							...
							### maya 2010 bugfix - can't set rotation values with setKeyframe when on animLayers
								...
								#try:
									#cmds.setAttr(obj + "." + attr, float(self.dataObj.getKeyAnimData(objID, attr, key, 'value')))
									#cmds.setKeyframe(obj , time =  frameNr  , attribute = str(attr), breakdown=False, hierarchy='none', controlPoints=False, shape=False )
								#except:
									#pass
									
							#else:
								#cmds.setKeyframe(obj , time =  frameNr  , attribute = str(attr)  , value =  float(self.dataObj.getKeyAnimData(objID, attr, key, 'value')), breakdown=False, hierarchy='none', controlPoints=False, shape=False )
						...
						# Set Tangents
						cmds.keyTangent(obj + '.' + attr, edit=True, wt = int(self.dataObj.getAttrData(objID, attr, 'weightedTangents')))
						...
						for keyID in self.dataObj.getAttrKeyID(objID , attr):
							...
							try:
								...
								cmds.keyTangent(obj + '.' + attr, time=( frameNr ,frameNr ), edit=True, inAngle = inAngleVal, outAngle = outAngleVal, inWeight = inWeightVal, outWeight = outWeightVal)
								cmds.keyTangent(obj + '.' + attr, time=( frameNr ,frameNr ), edit=True, inTangentType = inTangentTypeVal, outTangentType = outTangentTypeVal)
								cmds.keyTangent(obj + '.' + attr, time=( frameNr ,frameNr ), edit=True, lock = lockVal)
								if int(self.dataObj.getAttrData(objID, attr, 'weightedTangents')):
									...
									cmds.keyTangent(obj + '.' + attr, time=( frameNr ,frameNr ), edit=True, weightLock = weightLockVal)
							except:
...
class ProgressHandler:
	...
	def __init__(self, steps, jobType ):
		...
		if platformCase == "ms":
			self.os = "windows"
			cmds.progressWindow(title = jobType, progress = 0, status = "Initializing progress bar", isInterruptable = False)
	...
	def __windowsPrint(self):
		cmds.progressWindow(edit = True, progress = int(self.curStatus), status = "Progress is "+ str(int(self.curStatus)).zfill(2) + "% done" )
		self.__next()
...
class PaieGUI:
	def __init__(self):
		...
		defaultDir = cmds.internalVar(userAppDir=True)

		if cmds.optionVar( arraySize = self.optVar_path ) == 0:
			cmds.optionVar( stringValueAppend=(self.optVar_path, defaultDir) )
	...
	def makeGUI(self):
		...
		if cmds.window(guiName, exists=True):
			cmds.deleteUI(guiName)
			self.resetUiPaths()
		
		cmds.window(guiName, t="PAIE - Python Attribute Import/Export, v" + paieVersion)
		self.uiPath_mainForm = cmds.formLayout(numberOfDivisions=100)
		
		self.uiPath_topBar = cmds.rowLayout(parent = self.uiPath_mainForm, nc=4, adjustableColumn=1, columnAttach4=("both", "both", "both", "both"), columnAlign4=("left", "center", "center", "center") )
		
		cmds.radioButtonGrp(columnWidth3=(60, 80, 80), numberOfRadioButtons=2, label="Mode:", labelArray2=("Import", "Export"), select = self.mode, on1 = lambda *args: self.setMode(1), on2 = lambda *args: self.setMode(2))
		cmds.button( label="Refresh Files", width=100, command = lambda *args: self.updateFilelist() )
		cmds.button( label="Add Tab", width=100, command = lambda *args: self.addNewTab() )
		cmds.button( label="Remove Tab", width=100, command = lambda *args: self.removeTab() )
		
		self.uiPath_tabs = cmds.tabLayout(parent = self.uiPath_mainForm, changeCommand= lambda *args: self.tabChanged())
		
		
		cmds.formLayout( self.uiPath_mainForm, edit=True,
			attachForm=[
		...
		cmds.tabLayout(self.uiPath_tabs, edit=True, childResizable=True, selectTabIndex = self.currentTab+1)  # +1 when editing tabLayout. Stupid 1-indexing!
		self.updateFilelist() 
		
		if not cmds.windowPref(guiName, exists=True):
			if platformCase == "gnu":
				cmds.window(guiName, edit=True, height = self.listHeight + 150, width=500)
			else:
				cmds.window(guiName, edit=True, height = self.listHeight + 200, width=500)
		
		cmds.showWindow(guiName)
		
		width = cmds.window(guiName, query=True, width=True)
		cmds.window(guiName, edit=True, width=width + 2);cmds.window(guiName, edit=True, width=width)
	...
	def makeTab(self, index ):
		...
		tabCol = cmds.columnLayout(adj=1, rowSpacing=5, parent = self.uiPath_tabs)
		
		pathRow = cmds.rowLayout(nc=3, adjustableColumn=2, columnAlign=(3, "center"), columnAttach=(1, "left", 5), columnWidth=((1, 80), (3, 60)) )
		
		cmds.text(label ="Directory:", width=75)
		self.uiPath_pathField[ index ] = cmds.textField( text = self.getCurrentPath(), changeCommand = lambda *args: self.setNewPath( *args ) )
		cmds.button(label="Browse", width=55, command = lambda *args: self.browseFolders())
		

		if self.mode == 1: #Import
			checkRow = cmds.rowLayout( parent=tabCol, nc=4, columnWidth4=(80,130, 170, 150) )
			
			cmds.text( label="", width=80, height=28)
			self.uiPath_selectOrder[index] = cmds.checkBox(label="Select Order", value = self.selectOrder[index], changeCommand = lambda *args: self.setSelectOrder(*args))
			self.uiPath_origNamespaces[index] = cmds.checkBox(label="Use Original Namespaces", value = self.origNamespaces[index], enable = False, changeCommand = lambda *args: self.setOrigNamespace(*args))
			self.uiPath_applyAtOrigin[index] = cmds.checkBox(label="Apply At Origin", value = self.applyAtOrigin[index], ann = "Imports animation onto original frame numers", enable = True, changeCommand = lambda *args: self.setApplyAtOrigin(*args))
			
			listForm = cmds.formLayout(parent= tabCol, numberOfDivisions=100)
			
			group1 = cmds.columnLayout( parent = listForm, adj = 1)
			
			cmds.text(label="Files", font="boldLabelFont")
			self.uiPath_fileList[index] = cmds.textScrollList(height = self.listHeight, deleteKeyCommand = lambda *args: self.deleteSelectedFile(), selectCommand = lambda *args: self.fileSelected() )
			
			group2 = cmds.columnLayout( parent = listForm, adj = 1)
			
			cmds.text(label="Namespaces", font="boldLabelFont")
			self.uiPath_namespaceList[index] = cmds.textScrollList(height = self.listHeight )
			
			group3 = cmds.columnLayout( parent = listForm, adj = 1)
			
			cmds.text(label="File Info", font="boldLabelFont")
			self.uiPath_fileInfo[index] = cmds.scrollField( height = self.listHeight - 28, editable=False, ww=1, font="smallFixedWidthFont")
			cmds.button(label = "Import", align = "center", command = Callback(self.importButton) )
			#cmds.button(label="Import", align="center", command = __name__ + '.importButton("' + str(self.uiPath_fileList[ index ]) + '", "' + str(self.uiPath_namespaceList[ index ]) + '", "' + str(self.optVar_path) + '", "' + str(self.uiPath_selectOrder[index]) + '", ' + str(index) + ', "' + str(self.fileExt) + '")' )
		
			cmds.formLayout(listForm, edit=True, 
				attachForm=[
					(group1, "top", 5),
					(group1, "left", 5),
					(group1, "bottom", 5),
			...
			checkRow = cmds.rowLayout( parent=tabCol, nc=3, columnAttach=(1, 'left', 5), columnWidth3=(250, 320, 50) )
			
			self.uiPath_filename[ index ] = cmds.textFieldGrp( label="Filename", columnAlign=(1, 'left'), columnWidth2=(75,130) )
			self.uiPath_exportType[ index ] = cmds.radioButtonGrp( select = self.exportType[ index ], columnWidth=(1, 100), columnAlign=(1, "left"), on1 = lambda *args: self.setExportType(1), on2 = lambda *args: self.setExportType(2), label = "Export Type", labelArray2 = ["Animation", "Pose"], numberOfRadioButtons = 2)
			self.uiPath_exportTimeline[index] = cmds.checkBox(label="Timeline Only", value = self.exportTimeline[index], ann = "Exports animation on timeline only (instead of all animation on object)", enable = True, changeCommand = lambda *args: self.setExportTimeline(*args))
			
			listForm = cmds.formLayout(parent= tabCol, numberOfDivisions=100)
			
			group1 = cmds.columnLayout( parent = listForm, adj = 1)
			
			cmds.text(label="Existing Files", font="boldLabelFont")

			self.uiPath_fileList[index] = cmds.textScrollList(height = self.listHeight, deleteKeyCommand = lambda *args: self.deleteSelectedFile(), selectCommand = lambda *args: self.fileSelected() )
			
			group3 = cmds.columnLayout( parent = listForm, adj = 1)
			
			cmds.text(label="Comments", font="boldLabelFont")
			self.uiPath_comments[index] = cmds.scrollField( height = self.listHeight - 28, ww=1, font="smallFixedWidthFont")
			self.uiPath_exportButton[ index ] = cmds.button(label="Export", align="center", command= lambda *args: self.exportButton() )
	...
	def setExportEnable(self):
		...
		if self.mode == 2:
			cmds.textFieldGrp( self.uiPath_filename[ self.currentTab ], edit=True, editable= permissions)
			cmds.radioButtonGrp( self.uiPath_exportType[ self.currentTab ], edit=True, editable= permissions)
			cmds.textScrollList( self.uiPath_fileList[ self.currentTab ], edit=True, enable= permissions)
			cmds.scrollField( self.uiPath_comments[ self.currentTab ], edit=True, editable= permissions)
			cmds.button( self.uiPath_exportButton[ self.currentTab ], edit=True, enable= permissions)
		...
		if inputPath != None and inputPath != "":
			...
			if not os.path.isdir( inputPath ):
				...
				inputPath = self.getCurrentPath()
                                ...
                                if not os.path.isdir( inputPath ):
                                    inputPath = cmds.internalVar(userAppDir=True)
			
			
			if cmds.optionVar( arraySize = self.optVar_path ) < 2:
				pathArray = [inputPath]
	
			else:
				pathArray = cmds.optionVar(q=self.optVar_path)
				
			if self.currentTab < len(pathArray):
				# updating tab optionVar
				pathArray[ self.currentTab ] = inputPath
				cmds.optionVar( clearArray = self.optVar_path)
				for path in pathArray:
					cmds.optionVar(stringValueAppend=( self.optVar_path, path) )
					
				self.setExportEnable()
				self.updateFilelist()
					
				self.setTabLabel()
				cmds.textField(self.uiPath_pathField[ self.currentTab ], edit=True, text = inputPath) 
	...
	def setTabLabel(self ):
		...
		if len(split) > 1:
			...
			label = "None"
		cmds.tabLayout(self.uiPath_tabs, edit=True, tabLabelIndex = [self.currentTab +1, label] )
	...
	def getCurrentPath(self):
		...
		size = cmds.optionVar(arraySize = self.optVar_path)
		pathList = cmds.optionVar(query = self.optVar_path)
	...
	def getPathCount(self):
		pathCount = cmds.optionVar( arraySize = self.optVar_path)
		return pathCount
	...
	def deleteSelectedFile(self):
		...
		filename = cmds.textScrollList( self.uiPath_fileList[ self.currentTab ], query=True, selectItem=True )[0]
		...
		if proceed == 1:
			...
			if permissions == 0:
				...
				answer = cmds.confirmDialog( title="Delete File", message = "Are you sure you want to delete this file?", button = ["Yes", "No"], defaultButton = "Yes", cancelButton = "No", dismissString = "No")
				if answer == "Yes":
					os.remove(filepath)
					curSel = cmds.textScrollList( self.uiPath_fileList[ self.currentTab ], query=True, selectIndexedItem=True)[0]
					cmds.textScrollList( self.uiPath_fileList[ self.currentTab ], edit = True, removeItem = filename)
					
					if curSel > 1:
						curSel -= 1
					
					curSel = cmds.textScrollList( self.uiPath_fileList[ self.currentTab ], edit=True, selectIndexedItem = curSel)
                                        self.fileSelected()
                                        #if self.mode == 0:
                                        #    cmds.textScrollList( self.uiPath_namespaceList[ self.currentTab ], edit = True, removeAll = True)
	...
	def addNewTab(self):
		defaultPath = cmds.internalVar(userAppDir=True)
		cmds.optionVar(stringValueAppend = (self.optVar_path, defaultPath))
		self.addListElement()
		pathCount = self.getPathCount()
		self.makeTab( pathCount - 1 )
		cmds.tabLayout(self.uiPath_tabs, edit=True, selectTabIndex = self.currentTab+1)
	...
	def removeTab(self):
		...
		if pathArraySize == 0 or pathArraySize == 1:
			...
			cmds.optionVar(removeFromArray = (self.optVar_path, self.currentTab) )
	...
	def refreshGUI(self):
		...
		cmds.evalDeferred( lambda *args: self.makeGUI() )
		cmds.evalDeferred( lambda *args: self.__updateWidth() )
		
		
	def __updateWidth(self):
		if platformCase == "gnu": 	# Forcing update on Linux
			oldWidth = cmds.window("PaieGUI", query=True, width=True)
			cmds.evalDeferred('cmds.window("PaieGUI", edit=True, width= ' + str(oldWidth + 1) + ' )')
			cmds.evalDeferred('cmds.window("PaieGUI", edit=True, width= ' + str(oldWidth) + ' )')
	...
	def browseFolders(self):
		...
		if platformCase == "ms":
			...
			mayaWorkspace = cmds.workspace(q=True, dir=True)
			cmds.workspace(dir= self.getCurrentPath() )
			cmds.fileBrowserDialog(m = 4, fc = lambda *args: self.applyBrowsePath(*args), ft="directory", an="Set Work Dir", om="Import")
			cmds.workspace(dir= mayaWorkspace)
	...
	def fileSelected(self, *args):
		selFile = cmds.textScrollList( self.uiPath_fileList[ self.currentTab ], query=True, selectItem=True )
		...
		if selFile != None:
			...
			if self.mode == 1: # Import
				...
				if loadVal:
					...
					cmds.textScrollList(self.uiPath_namespaceList[ self.currentTab ], edit=True, removeAll=True)
					if len(namespaces) == 1:
						cmds.textScrollList( self.uiPath_namespaceList[ self.currentTab ], edit = True, append = namespaces[0])
						cmds.textScrollList( self.uiPath_namespaceList[ self.currentTab ], edit = True, selectItem = namespaces[0])
					else:
						for ns in namespaces:
							cmds.textScrollList( self.uiPath_namespaceList[ self.currentTab ], edit=True, append= ns)
						
					cmds.scrollField( self.uiPath_fileInfo[ self.currentTab ], edit=True, clear=True)
					for infoLine in fileInfo:
						cmds.scrollField( self.uiPath_fileInfo[ self.currentTab ], edit=True, insertText = infoLine + "\n" )
		
				else: # For unsupported struct versions
					cmds.textScrollList(self.uiPath_namespaceList[ self.currentTab ], edit=True, removeAll=True)
					cmds.scrollField( self.uiPath_fileInfo[ self.currentTab ], edit=True, clear=True)
					cmds.scrollField( self.uiPath_fileInfo[ self.currentTab ], edit=True, insertText = "Unsupported Version \nThis file cannot be used\nFile Version:\t" + str(wrapperObj.loadedVersion) )
					
					
			else: # Export
				cmds.textFieldGrp( self.uiPath_filename[ self.currentTab ], edit=True, text = selFile )
		else:
	...
	def updateFilelist(self):
		...
		if paieFiles != 0:
			cmds.textScrollList(self.uiPath_fileList[ self.currentTab ], edit=True, removeAll=True)
			for f in paieFiles:
				cmds.textScrollList(self.uiPath_fileList[ self.currentTab ], edit = True, append = f)
		
	
		
		
	def tabChanged(self):
		selectedTab = cmds.tabLayout(self.uiPath_tabs, q=True, selectTabIndex=True)
	...
	def importButton(self):
		...
		path = self.getCurrentPath()
		selFileList = cmds.textScrollList( self.uiPath_fileList[ self.currentTab ], query=True, selectItem=True )
		selNamespaceList = cmds.textScrollList( self.uiPath_namespaceList[ self.currentTab ], query=True, selectItem=True )
	...
	def exportButton(self):
		comments = cmds.scrollField(self.uiPath_comments[ self.currentTab ], query=True, tx=True)
		filename = cmds.textFieldGrp(self.uiPath_filename[ self.currentTab ], query=True, tx=True)
		try:
...
def __fixPath(filePath):
	...
	if filePath == '':
		...
		return (cmds.internalVar(userAppDir=True) + 'paieDefault.' + "xad")
...
def __checkFile(filePath):
	...
	if proceed == 1:
		proceed = cmds.confirmDialog( title="File exist", message = "File already exist. Overwrite it?", button = ["Yes", "No"], defaultButton = "Yes", cancelButton = "No", dismissString = "No")
...
def __getSelection():
	curSelection = cmds.ls(sl=True, long=True)
	...
	if type(curSelection) != type([]):
		...
		try:
			...
			# Expand through sets
			cmds.select(curSelection, r=True)
			selWithSets = cmds.ls(sl=True, long=True)
			
			# Revert to original selection
			cmds.select(curSelection, noExpand=True)
...
def exportData(filePath, dataType, attrsType='keyable', exportTimeline=1, startFrame=None, endFrame=None, objs=None, userInput=None):
	...
	try:	
		...
		if exportTimeline == 1:
			...
			if startFrame == None:
				startFrame = cmds.findKeyframe(objs, which='first')
			if endFrame == None:
				endFrame = cmds.findKeyframe(objs, which='last')
		...
		cmds.confirmDialog( title='Error', message=arg.__str__(), button='OK' )
		...
		cmds.confirmDialog( title='Error', message="# exportData >> Non-unicode characters aren't supported. Stop using them!", button='OK' )
		return 0
			...
			print "# importData >> File was empty. Wtf?!",
                        ...
                        if startFrame == None:
                            if applyAtOrigin:
                                ...
                                startFrame = cmds.currentTime(query=True)
